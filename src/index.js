...
const LENGTH_RULE_MAP = {
    "Очень короткие": (maxTokens) =>
        `СТРОГОЕ ПРАВИЛО ДЛИНЫ: Отвечай максимально кратко — как живой человек в мессенджере. ` +
        `1-2 короткие фразы, не более ${maxTokens} токенов. ` +
        `Никаких длинных описаний, размышлений или внутренних монологов. ` +
        `Если можно ответить одним словом — отвечай одним словом.`,
    "Короткие": (maxTokens) =>
        `ПРАВИЛО ДЛИНЫ: Отвечай кратко — 1-3 предложения, не более ${maxTokens} токенов. ` +
        `Не добавляй лишних деталей, если пользователь не просит.`,
    "Средние": (maxTokens) =>
        `ПРАВИЛО ДЛИНЫ: Отвечай средней длины — 3-6 предложений, до ${maxTokens} токенов. ` +
    ...
    "Длинные": (maxTokens) =>
        ...
        `До ${maxTokens} токенов. Не укорачивай без необходимости.`,
};
